...
router.get("/dashboard", Authenticate, requireAdmin, async (req, res) => {
	try {
		...
		const reportedSwaps = await SwapRequest.countDocuments({
			isReported: true,
		});
...
router.get("/reports/:type", Authenticate, requireAdmin, async (req, res) => {
	try {
		...
		switch (type) {
			case "users":
				const users = await User.find({
					...
					"fullName email username location skillsOffered skillsWanted isProfilePublic isBanned createdAt"
				);

				csvData =
					"Full Name,Email,Username,Location,Skills Offered,Skills Wanted,Profile Public,Banned,Registration Date\n";
				users.forEach((user) => {
					csvData += `"${user.fullName}","${user.email}","${
						user.username
					}","${user.location || ""}","${user.skillsOffered
						.map((s) => s.name)
						.join("; ")}","${user.skillsWanted
						.map((s) => s.name)
						.join("; ")}","${user.isProfilePublic}","${
						user.isBanned
			...
			case "swaps":
				const swaps = await SwapRequest.find({
					...
					.populate("requested", "fullName email");

				csvData =
					"Requester Name,Requester Email,Requested Name,Requested Email,Skill Offered,Skill Wanted,Status,Created Date,Completed Date,Rating\n";
				swaps.forEach((swap) => {
					csvData += `"${swap.requester?.fullName || "N/A"}","${
						swap.requester?.email || "N/A"
					}","${swap.requested?.fullName || "N/A"}","${
						swap.requested?.email || "N/A"
					}","${swap.skillOffered}","${swap.skillWanted}","${
						swap.status
					}","${swap.createdAt.toISOString().split("T")[0]}","${
						swap.completedAt
							? swap.completedAt.toISOString().split("T")[0]
							: "N/A"
					}","${swap.rating || "N/A"}"\n`;
				});
				break;
			...
			case "activity":
				...
				csvData += `"Completion Rate","${
					totalSwaps > 0
						? ((completedSwaps / totalSwaps) * 100).toFixed(2) + "%"
						: "0%"
				}","Last 30 Days"\n`;
				break;
			...
			case "admin":
				...
				adminMessages.forEach((message) => {
					csvData += `"Platform Message","${message.title}","${
						message.message
					}","${message.createdBy?.fullName || "System"}","${
						message.createdAt.toISOString().split("T")[0]
					}"\n`;
				});
				bannedUsers.forEach((user) => {
					csvData += `"User Banned","${user.fullName}","Username: ${
						user.username
		...
		res.setHeader(
			"Content-Disposition",
			`attachment; filename="${type}_report_${
...
router.delete('/reject-skill/:id', Authenticate, requireAdmin, async(req, res)=>{
    const {id} = req.params
